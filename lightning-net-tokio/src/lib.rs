...
impl Connection {
		...
		PM::Target: APeerManager<Descriptor = SocketDescriptor>,
	{
		...
		spawn_inherit(Self::poll_event_process(peer_manager.clone(), event_receiver));
		...
		let disconnect_type = loop {
			...
			match select_result {
				SelectorOutput::A(v) => {
					if v.is_none() {
						print_and_flush_error("write_avail_receiver dropped!").await;
					}
...
/// Exactly [`tokio::spawn`], but spawned futures inherit the current span.
// Use fully qualified names and 0 imports to reduce rebase conflicts
pub fn spawn_inherit<T>(future: T) -> tokio::task::JoinHandle<T::Output>
where
	T: Future + Send + 'static,
	T::Output: Send + 'static,
{
	tokio::spawn(tracing::Instrument::in_current_span(future))
}
...
/// HACK: Print an error and try to ensure it gets flushed before proceeding.
/// This is because backtraces are currently getting swallowed by SGX.
/// We'll also sleep so tracing logs have a chance to be flushed before we exit.
async fn print_and_flush_error(error: impl Display) {
...
mod tests {
	...
	async fn race_disconnect_accept() {
		...
		super::spawn_inherit(async move { super::setup_inbound(manager_reference, conn_a).await });
		super::spawn_inherit(async move { super::setup_outbound(a_manager, b_pub, conn_b).await });
	}
