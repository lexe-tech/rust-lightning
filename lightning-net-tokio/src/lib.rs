...
impl Connection {
	...
	async fn schedule_read<E, PM: Deref + 'static + Send + Sync + Clone>(
		executor: E, peer_manager: PM, us: Arc<Mutex<Self>>, reader: Arc<TcpStream>,
		...
		PM::Target: APeerManager<Descriptor = SocketDescriptor>,
	{
		...
		let future = Self::poll_event_process(peer_manager.clone(), event_receiver);
...
/// A trait for something which executes futures without requiring the user to
/// manually poll them. Concretely, this should always wrap a `tokio::spawn`
/// call, but is abstracted to allow users to customize spawning behavior.
pub trait Executor<F: Future> {
	/// Place the given future into the executor to be run.
	fn execute(&self, fut: F) -> tokio::task::JoinHandle<F::Output>;
}

/// A default [`Executor`] which just calls [`tokio::spawn`].
#[derive(Clone)]
...
where
	PM::Target: APeerManager<Descriptor = SocketDescriptor>,
{
	setup_inbound_with_executor(TokioExecutor, peer_manager, stream)
}
	...
	PM::Target: APeerManager<Descriptor = SocketDescriptor>,
{
	...
	let handle_opt =
		if peer_manager.as_ref().new_inbound_connection(descriptor, remote_addr).is_ok() {
			let future = Connection::schedule_read(
				executor.clone(),
				peer_manager,
				us,
				reader,
			...
			let handle = executor.execute(Box::pin(future));
			Some(handle)
		} else {
...
where
	PM::Target: APeerManager<Descriptor = SocketDescriptor>,
{
	setup_outbound_with_executor(TokioExecutor, peer_manager, their_node_id, stream)
}
